...
class NCQSymFunction:
    def __init__(self, *args, **kwargs):
        ...
        if "monomial_basis" in kwargs:
            ...
            if isinstance(item, (str, SetComposition)):
                ...
                # Copy the dict to prevent external mutation from affecting this object
                self.coefficients = dict(item)
            else:
    ...
    def __add__(self, g: 'NCQSymFunction') -> 'NCQSymFunction':
        # Copy self.coefficients so the new object is independent
        answer = NCQSymFunction(monomial_basis=dict(self.coefficients))
        answer._addTo(g)
        return answer
    ...
    def _scalarMultiple(self, scalar: int) -> 'NCQSymFunction':
        # Copy self.coefficients so the new object is independent
        answer = NCQSymFunction(monomial_basis=dict(self.coefficients))
        for tstr in answer.coefficients:
    ...
    def __mul__(self, g: 'NCQSymFunction') -> 'NCQSymFunction':
        ...
        for tstr, tcoeff in self.coefficients.items():
            t = SetComposition(tstr)
            for qstr, qcoeff in g.coefficients.items():
                q = SetComposition(qstr)
                coeff = qcoeff * tcoeff
                shuffles = SetComposition.quasi_shuffles(t, q)
                answer._addTo(NCQSymFunction(monomial_basis=shuffles)._scalarMultiple(coeff))
        return answer
